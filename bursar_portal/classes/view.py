...
from pos_portal.models import StudentAccountsWallets,Transactions
...
from bursar_portal.forms import StudentBalanceForm,DepositForm,WithdrawForm,ActivateCardForm
...
def index(request):
	...
	try:
		...
		pass
	return render(request, 'bursar_portal/index.html', ctx)


def check_balance(request):
	ctx = {}
	form = StudentBalanceForm(request.POST or None)
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/check_balance.html', ctx)
...
def get_balance(request):
	...
	if request.is_ajax():
		...
		walletname_name = request.POST.get('walletname_name')
		studentaccountswallets = StudentAccountsWallets.objects.get(studentprofiles_id=studentprofiles,walletname_name=walletname_name)
		transactions = Transactions.objects.filter(wallet_id=studentaccountswallets.id).order_by('id')[:1]
		data = {'balance': transactions[0].balance}
		return HttpResponse(json.dumps(data), content_type='application/json')
	else:
		#rows = Pos.objects.all()
		#ctx['rows'] = rows
...
def deposit(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/deposit.html', ctx)


def do_deposits(request):
	ctx = {}
	if request.is_ajax():
		studentprofiles = request.POST.get('studentprofiles')
		walletname_name = request.POST.get('walletname_name')
		...
		transactions = Transactions(wallet_id=walletname_name,transaction_type="Deposit",amount=amount,currency='KSH',location_id=1,terminal_id='1',balance=0,reverse=0)
		transactions.save()

		studentaccountswallets = StudentAccountsWallets.objects.get(studentprofiles_id=studentprofiles,pk=transactions.wallet_id)
		transactions = Transactions.objects.filter(wallet_id=studentaccountswallets.id).order_by('id')[:1]
		data = {'balance': transactions[0].balance}
		return HttpResponse(json.dumps(data), content_type='application/json')
	else:
...
def withdraw(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/withdraw.html', ctx)


def do_withdraw(request):
	ctx = {}
	if request.is_ajax():
		studentprofiles = request.POST.get('studentprofiles')
		#walletname_name = request.POST.get('walletname_name')
		...
		transactions = Transactions(wallet_id=2,transaction_type="Deposit",amount=amount,currency='KSH',location_id=1,terminal_id='1',balance=0,reverse=0)
		transactions.save()
...
def activatecard(request):
	ctx = {}
	form = ActivateCardForm(request.POST or None)
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/activatecard.html', ctx)


def do_activatecard(request):
	ctx = {}
	if request.is_ajax():
		studentprofiles = request.POST.get('studentprofiles')
		walletname_name = request.POST.get('walletname_name')
		...
		studentaccountswallets = StudentAccountsWallets.objects.get(studentprofiles_id=studentprofiles,walletname_name=walletname_name)
		studentaccountswallets.is_locked=iss
		studentaccountswallets.save()
...
def register(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/register.html', ctx)
...
def syssettings(request):
	...
	if request.is_ajax():
		...
		ctx['dormitoryform'] = dormitoryform
		ctx['span_size'] = 12
		return render(request, 'bursar_portal/syssettings.html', ctx)
...
def studentclubs(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/studentclubs.html', ctx)
...
def holidayjobs(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/holidayjobs.html', ctx)
...
def studentsports(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/studentsports.html', ctx)
...
def discipline(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/disciplinecases.html', ctx)
...
def studentsupplies(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/studentsupplies.html', ctx)
...
def certsofachievment(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/certsofachievment.html', ctx)
...
def academics(request):
		...
		return render(request, 'bursar_portal/academics.html', ctx)
...
def library(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/library.html', ctx)
...
def books(request):
	...
	if request.is_ajax():
		...
		return render(request, 'bursar_portal/books.html', ctx)
...
def book_create(request):
	if request.is_ajax() and request.POST:
		...
		try:
			ss=SchoolStaff.objects.get(owner_id=request.user.id)
			obj=Books(name_id=ss.name_id,bookname=bookname,author=author,category=category,boughtsponsored=boughtsponsored,amount=amount);
			obj.save()
			pkid=obj.id
